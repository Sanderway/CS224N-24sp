...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    ### TODO
    scores = torch.matmul(query, key.transpose(-2,-1)) / math.sqrt(self.all_head_size) # (b,h,n,dk)
    ...
    return weighted_values.flatten(2)
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    ### TODO
    output_add_norm = ln_layer(dropout(dense_layer(output)) + input)
    return output_add_norm
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    attention_addnorm = self.add_norm(hidden_states,attention_output,self.attention_dense,self.attention_dropout,self.attention_layer_norm)
    ff_output = self.interm_dense(attention_addnorm)
    ff_output = self.interm_af(ff_output)
    output = self.add_norm(attention_addnorm,ff_output,self.out_dense,self.out_dropout,self.out_layer_norm)
    return output
